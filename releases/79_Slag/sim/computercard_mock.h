...
// src/slag.cpp can be compiled and run on a normal computer (Mac / Linux /
...
// Make the real (Pico-only) ComputerCard.h self-skip when the card includes
// it: its guard is #ifndef COMPUTERCARD_H.
#define COMPUTERCARD_H
// Tell the card source to omit its hardware main().
...
public:
	...
	bool     simPulseOut[2] = { false, false };
	...
	void PulseOut(int i, bool v)    { simPulseOut[i] = v; }
	void PulseOut1(bool v)          { simPulseOut[0] = v; }
	void PulseOut2(bool v)          { simPulseOut[1] = v; }
