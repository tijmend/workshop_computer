...
#if WS_COMPUTER
// 8-bit palette-index state for the USB frame encoder
static uint8_t ws_pal_rgb[768];
static volatile boolean ws_pal_dirty;
static uint8_t ws_shared_pal8[NUM_SHARED_PALETTES][16];
static inline void ws_pal_store(int i, int r, int g, int b) {
    ws_pal_rgb[i * 3] = r;
    ws_pal_rgb[i * 3 + 1] = g;
    ws_pal_rgb[i * 3 + 2] = b;
    ws_pal_dirty = true;
}
// a host that attaches mid-game missed the last palette packet: mark it
// dirty so the encoder sends it before the next frame
void ws_video_resend_palette(void) {
    ws_pal_dirty = true;
}
#endif
...
#if WS_COMPUTER
// no video hardware: frames are consumed by the USB encoder instead
#elif USE_1280x1024x60
    ...
    if (display_video_type >= FIRST_VIDEO_TYPE_WITH_OVERLAYS) {
        ...
        if (next_pal != -1) {
            ...
            for (int i = 0; i < NUM_SHARED_PALETTES; i++) {
                ...
                for (int j = 0; j < 16; j++) {
#if WS_COMPUTER
                    ws_shared_pal8[i][j] = vpatch_palette(patch)[j];
#endif
...
#if WS_COMPUTER
// ---------------------------------------------------------------------
// Workshop Computer USB frame encoder. Runs synchronously on core 0 from
// the display_frame_freed spin sites in pd_render.cpp, so nothing renders
// into a buffer while it is being encoded.

#include "ws/ws_stream.h"

// draw one scanline's worth of a vpatch as 8-bit palette indices
// (index-space mirror of draw_vpatch above)
static uint ws_draw_vpatch8(uint8_t *dest, patch_t *patch, vpatchlist_t *vp, uint off) {
    int repeat = vp->entry.repeat;
    dest += vp->entry.x;
    int w = vpatch_width(patch);
    const uint8_t *data0 = vpatch_data(patch);
...
static void ws_build_line(int y, uint8_t *line) {
    const uint8_t *src;
    switch (display_video_type) {
        case VIDEO_TYPE_DOUBLE:
            if (y < MAIN_VIEWHEIGHT) {
                memcpy(line, frame_buffer[display_frame_index] + y * SCREENWIDTH, SCREENWIDTH);
            } else {
                memset(line, 0, SCREENWIDTH); // status bar overlays draw over this
            }
            break;
        case VIDEO_TYPE_SAVING:
        case VIDEO_TYPE_SINGLE:
            if (y < MAIN_VIEWHEIGHT) {
                src = frame_buffer[display_frame_index] + y * SCREENWIDTH;
            } else {
                src = frame_buffer[display_frame_index ^ 1] + (y - 32) * SCREENWIDTH;
            }
            memcpy(line, src, SCREENWIDTH);
            break;
        case VIDEO_TYPE_WIPE: {
            ...
            for (int i = 0; i < SCREENWIDTH; i++) {
                int rel = y - wipe_yoffsets[i];
                if (rel < 0) {
                    line[i] = src[i];
                } else {
                    const uint8_t *flip = (const uint8_t *) wipe_linelookup[rel];
                    if (flip >= &frame_buffer[0][0] &&
                        flip < &frame_buffer[0][0] + 2 * SCREENWIDTH * MAIN_VIEWHEIGHT) {
                        line[i] = flip[i];
                    }
                }
            }
    ...
    if (display_video_type >= FIRST_VIDEO_TYPE_WITH_OVERLAYS) {
        // same overlay bookkeeping as fill_scanlines, drawing indices
        assert(y < count_of(vpatchlists->vpatch_starters));
        int prev = 0;
        ...
        for (int vp = vpatchlists->vpatch_next[prev]; vp; vp = vpatchlists->vpatch_next[prev]) {
            ...
            if (yoff < vpatch_height(patch)) {
                vpatchlists->vpatch_doff[vp] = ws_draw_vpatch8(line, patch, &overlays[vp],
                                                              vpatchlists->vpatch_doff[vp]);
                prev = vp;
            } else {
                vpatchlists->vpatch_next[prev] = vpatchlists->vpatch_next[vp];
...
static void ws_send_line(int y, const uint8_t *line) {
    uint8_t pkt[1 + 2 * SCREENWIDTH];
    int n = 0;
    pkt[n++] = (uint8_t) y;
    int i = 0;
    while (i < SCREENWIDTH) {
        uint8_t v = line[i];
        int run = 1;
        while (i + run < SCREENWIDTH && line[i + run] == v && run < 255) run++;
        pkt[n++] = (uint8_t) run;
        pkt[n++] = v;
        i += run;
    }
    ws_stream_packet(WS_PKT_LINE, pkt, n);
}

void ws_video_present(void) {
    boolean have_frame = sem_available(&render_frame_ready);
    new_frame_stuff();
    if (!have_frame || !initialized) return;
    {
        extern void ws_boot_stage(int led);
        static boolean first_present_done;
        if (!first_present_done) {
            first_present_done = true;
            ws_boot_stage(4);
        }
    }
    ws_stream_status_tick();
    if (!ws_stream_active()) return;
    if (display_video_type == VIDEO_TYPE_NONE || display_video_type == VIDEO_TYPE_TEXT) return;

    // throttle so a slow host doesn't drag the game below ~20fps
    static uint32_t last_ms;
    uint32_t now = to_ms_since_boot(get_absolute_time());
    if (now - last_ms < 50) return;
    last_ms = now;

    // a stalled host tore a packet mid-write: its parser state (and maybe
    // its palette) is garbage, so resend the palette once it drains
    {
        extern bool ws_stream_take_torn(void);
        if (ws_stream_take_torn()) ws_pal_dirty = true;
    }
    if (ws_pal_dirty) {
        ws_pal_dirty = false;
        ws_stream_packet(WS_PKT_PALETTE, ws_pal_rgb, 768);
    }
    static uint8_t line[SCREENWIDTH];
    for (int y = 0; y < SCREENHEIGHT; y++) {
        ws_build_line(y, line);
        ws_send_line(y, line);
    }
    static uint8_t frame_counter;
    frame_counter++;
    ws_stream_packet(WS_PKT_FRAME_END, &frame_counter, 1);
}
#endif // WS_COMPUTER

//static semaphore_t init_sem;
#if WS_COMPUTER
// headless: core 1 only assists the renderer (and mixes sound while idle);
// frame presentation happens on core 0 in ws_video_present()
static void core1() {
...
#if PICO_ON_DEVICE && WS_COMPUTER
// no scanvideo output on the Workshop Computer
#elif PICO_ON_DEVICE
bool video_doom_adapt_for_mode(const struct scanvideo_pio_program *program, const struct scanvideo_mode *mode,
                               struct scanvideo_scanline_buffer *missing_scanvideo_scanline_buffer, uint16_t *modifiable_instructions) {
