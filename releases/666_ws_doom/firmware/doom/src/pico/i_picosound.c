...
static void I_Pico_UpdateSound(void)
{
    ...
    if (buffer) {
#if WS_COMPUTER
        if (music_generator && !ws_ext_music()) {
#else
        if (music_generator) {
#endif
...
#if WS_COMPUTER
            {
                // positional music: the beacon logic pans/attenuates the
                // soundtrack toward the nearest enemy (128 = -6dB unity,
                // so effects always sit above the music)
                extern volatile int16_t ws_music_gain[2];
                int gl = ws_music_gain[0], gr = ws_music_gain[1];
                int16_t *ms = (int16_t *) buffer->buffer->bytes;
                for (uint s = 0; s < buffer->max_sample_count; s++) {
                    ms[0] = (int16_t) ((ms[0] * gl) >> 8);
                    ms[1] = (int16_t) ((ms[1] * gr) >> 8);
                    ms += 2;
                }
            }
#endif
...
#if WS_COMPUTER
                // effects level from the persisted user setting
                int voll = (channel->left * ws_sfx_vol) >> 9;
                int volr = (channel->right * ws_sfx_vol) >> 9;
#else
...
#if WS_COMPUTER
    // smaller buffers: frees 4K of zone that R_Init needs on this board
    producer_pool = audio_new_producer_pool(&producer_format, 2, 512);
#else
    producer_pool = audio_new_producer_pool(&producer_format, 2, 1024); // todo correct size
#endif

#if WS_COMPUTER
    // no I2S: the 48kHz MCP4822 DAC interrupt drains the producer pool
    // (give_audio_buffer's default connection queues into the pool itself)
    ws_audio_init(producer_pool);
#else
