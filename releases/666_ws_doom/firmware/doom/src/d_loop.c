...
static boolean BuildNewTic(void) {
...
#if WS_COMPUTER
    static int bnt_n;
    boolean bnt_v = bnt_n < 30;
    bnt_n++;
    if (bnt_v) ws_log("BNT %d: tic (mt=%d gt=%d)", bnt_n, maketic, gametic);
#endif
    ...
    I_StartTic();
#if WS_COMPUTER
    if (bnt_v) ws_log("BNT %d: ev", bnt_n);
#endif
    loop_interface->ProcessEvents();
#if WS_COMPUTER
    if (bnt_v) ws_log("BNT %d: menu", bnt_n);
#endif
...
#if WS_COMPUTER
    if (bnt_v) ws_log("BNT %d: cmd", bnt_n);
#endif
...
#if WS_COMPUTER
    if (bnt_v) ws_log("BNT %d: ok", bnt_n);
#endif
...
#if WS_COMPUTER
    {
        static int trt_logged;
        if (trt_logged < 6) {
            trt_logged++;
            ws_log("TRT: rt=%d avail=%d ct=%d gt=%d lt=%d",
                   realtics, availabletics, counts, gametic, lowtic);
        }
    }
    uint32_t ws_wait_spins = 0;
#endif
...
#if WS_COMPUTER
        if (++ws_wait_spins == 300000) {
            ws_wait_spins = 0;
            ws_log("TRT wait: lt=%d gt=%d ct=%d pig=%d",
                   lowtic, gametic / ticdup, counts, PlayersInGame());
        }
#endif
...
#if WS_COMPUTER
        {
            static int tic_logged;
            if (tic_logged < 8) {
                tic_logged++;
                ws_log("run tic gt=%d", gametic);
            }
        }
#endif
#if PICO_BUILD
...
#if WS_COMPUTER
    {
        static int trt_exit_logged;
        if (trt_exit_logged < 6) {
            trt_exit_logged++;
            ws_log("TRT exit gt=%d mt=%d", gametic, maketic);
        }
    }
#endif
#if PICO_BUILD
