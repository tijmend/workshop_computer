...
extern void ws_boot_phase2(void);
extern void ws_log(const char *fmt, ...);
// game start queued from inside D_DoAdvanceDemo (see D_DoomMain)
static boolean ws_autostart_pending;
#else
#define ws_boot_stage(led) ((void)0)
#define ws_boot_phase2() ((void)0)
#endif
...
#else
        {
            ...
            do {
                ...
                if (++wipe_guard > 5000) {
                    ...
                    ws_panic_fmt("wipe stuck: state %d min %d", wipestate, wipe_min);
                }
#endif
            } while (wipestate);
        }
...
#endif
    // phase 2 of the boot ladder: LEDs clear, then climb again while the
    // game loop starts up (first tic replaces this with live LED output)
    ws_boot_phase2();
    I_InitGraphics();
    ws_boot_stage(0);
#if USB_SUPPORT
...
#if WS_COMPUTER
        {
            static boolean first_frame_done;
            if (!first_frame_done) {
...
#if WS_COMPUTER
    ws_log("advdemo seq=%d", demosequence);
    // queue the module's game start now that the title state is set up —
    // queueing it any earlier gets wiped by the gameaction reset above
    if (ws_autostart_pending) {
        extern int ws_random_map(void);
        ws_autostart_pending = false;
        int m = ws_random_map();
        ws_log("starting at E1M%d", m);
        G_DeferedInitNew (startskill, startepisode, m, false);
    }
#endif
}
...
#if WS_COMPUTER
    // No screen on the module, so skip menu navigation and start a game.
    // It must be queued from inside D_DoAdvanceDemo: queueing it here gets
    // erased when the title sequence's first tic runs D_DoAdvanceDemo
    // (which sets gameaction = ga_nothing), and starting synchronously
    // would load the level before the renderer is initialized.
    D_StartTitle ();
    ws_autostart_pending = true;
#else
