...
#if WS_COMPUTER
// Spawn director for eyes-free play: keep one enemy within earshot.
// The first one drops 5s into the level; whenever no living monster is
// within ~1500 map units, the next (cycling zombieman / shotgunner /
// imp) appears ~5s later, in front of the player.
static void ws_action_spawn(void)
{
    static int next_spawn_time;
    static int spawn_cycle;
    extern volatile int32_t ws_nearest_dist;
    static const mobjtype_t ws_spawn_types[3] = { MT_POSSESSED, MT_SHOTGUY, MT_TROOP };

    if (leveltime == 0) {
        next_spawn_time = 5 * TICRATE; // opener is unconditional
        return;
    }
    if (leveltime > 6 * TICRATE) {
        // afterwards: only re-arm while nothing is close enough to hunt
        if (ws_nearest_dist < 1500) {
            next_spawn_time = 0;
            return;
        }
        if (next_spawn_time == 0) {
            next_spawn_time = leveltime + 5 * TICRATE;
            return;
        }
    }
    if (leveltime < next_spawn_time || (leveltime % TICRATE) != 0) {
        return;
    }
    ...
    // try a ring of positions around the player: many level starts face
    // a wall, and a single fixed spot can fail its placement check on
    // every retry
    angle_t pang = mobj_angle(pmo);
    for (int attempt = 0; attempt < 8; attempt++) {
        angle_t a = pang + (angle_t) attempt * ANG45;
        int an = a >> ANGLETOFINESHIFT;
        fixed_t dist = (attempt & 1) ? 96 * FRACUNIT : 128 * FRACUNIT;
        fixed_t x = pmo->xy.x + FixedMul(dist, finecosine(an));
        fixed_t y = pmo->xy.y + FixedMul(dist, finesine(an));
        mobj_t *m = P_SpawnMobj(x, y, ONFLOORZ, ws_spawn_types[spawn_cycle % 3]);
        if (!m) {
            break;
        }
        if (P_CheckPosition(m, m->xy.x, m->xy.y)) {
            mobj_full(m)->angle = a + ANG180; // face the player
            spawn_cycle++;
            // hold off until the beacon scan sees the newcomer
            next_spawn_time = leveltime + 5 * TICRATE;
            return;
        }
        P_RemoveMobj(m); // blocked; try the next spot
    }
    // all spots blocked; retry next second
}
#endif
