...
#if WS_COMPUTER
extern "C" void ws_video_present(void);
extern "C" void __attribute__((noreturn)) ws_panic_fmt(const char *fmt, ...);
extern "C" void ws_log(const char *fmt, ...);
// progress markers so a stuck core 1 can be located from a core 0 panic
volatile uint32_t ws_core1_stage, ws_core1_loops;
#endif
...
#if WS_COMPUTER
    // TinyUSB device with only a CDC interface allocates its endpoint
    // buffers below DPRAM offset ~0x2C0, so the region from +0x400 that
    // upstream uses is still free — main RAM goes to the zone instead
...
#if WS_COMPUTER
    if (!sem_acquire_timeout_ms(&core1_done, 3000)) {
        ws_panic_fmt("core1 stuck: stage %d loops %d flats=%d reg=%d c0d=%d",
                     (int) ws_core1_stage, (int) ws_core1_loops,
                     sem_available(&core1_do_flats), sem_available(&core1_do_regular),
                     sem_available(&core0_done));
    }
#else
...
#if WS_COMPUTER
#define WS_CORE1_STAGE(n) (ws_core1_stage = (n))
#else
#define WS_CORE1_STAGE(n) ((void)0)
#endif
