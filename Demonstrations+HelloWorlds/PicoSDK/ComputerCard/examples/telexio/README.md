...
By default the **computercard.h** framework seems to configure the UART pins as outputs during startup. Therefore, when using other cards based on computercard.h, disconnect the i2c connection to avoid the UART outputs fighting with the i2c-bus, or recompile those cards with the appropriate UART configuration flag - see my CMake file.
